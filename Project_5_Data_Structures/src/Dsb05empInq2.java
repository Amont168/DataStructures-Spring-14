...
// Date     :                 10/28/2013              10/10/2013 ~~ 12/1/13 
...
public class Dsb05empInq2
{
  public static void main(String[] args) throws IOException
  {
    dsb05empScreen empScrn = new dsb05empScreen();
...
//------------------------------------------------------------------------------
class dsb05empScreen extends JFrame
{
  private RandomAccessFile raf;
  private TreeBuildClass   treeObj;
  ...
  //-----------------------------------------------------------------constructor
  public dsb05empScreen() throws IOException
  {
    super("Dustyne Brown - Project 4");
    raf = new RandomAccessFile("emp-master.dat","r");
    
    try
      {
        ObjectInputStream input = new ObjectInputStream(new FileInputStream("emp-master.tree"));
        treeObj = (TreeBuildClass) input.readObject();
        input.close();
      }
    catch (ClassNotFoundException e) 
      {
        System.out.println("Unexpected Error: Data Tree Not Loaded!");
      }
    ...
    JButton     btnListBySSN  = new JButton("List By SSN");
    JPanel      pnlButtons    = new JPanel();
    pnlButtons.setLayout(new GridLayout(1,5)); //next to bottom row
    pnlButtons.add(btnReadPrevID);
    pnlButtons.add(btnReadNextID);
    pnlButtons.add(btnClear);
    pnlButtons.add(btnListAll);
    pnlButtons.add(btnListBySSN);
    ...
    btnListBySSN .addActionListener(new HandlerBySSN()     );
  ...
  private class HandlerBySSN implements ActionListener
  {
    public void actionPerformed(ActionEvent event)                     
    {                                                                  
      listInOrder(treeObj.root);                                       
    }                                                                  
    private void listInOrder(TreeBuildClass.TreeNode localRoot)        
    {                                                                  
      if(localRoot==null)                                              
        return;                                                        
                                                                       
      listInOrder(localRoot.lChild);                                   
                                                                       
      try                                                              
        {                                                              
          raf.seek(localRoot.trgt * REC_LEN);                          
          dsb04empRec empTrgt = readCurrRec();
          System.out.printf("%4s%12s%26s%9s%2s%15s%2s%3s\n", empTrgt.empnum, empTrgt.ssn, 
              empTrgt.name, empTrgt.phone, empTrgt.ptype, empTrgt.prate, empTrgt.mstat, empTrgt.exemp);
  ...
  private class HandlerSSN implements ActionListener
  {
    ...
    private long findSsn(String ssnReqStr, long targetLoc) throws IOException            //Method that hunts the 
      {                                                                                  // requested data
        ...
        if(ssnReqStr.length() > 18)
          {
            tfMsg.setText("Number entered too long!");
          }
        else
          {
            ssnReq = Long.parseLong(ssnReqStr);
            x = treeObj.find(ssnReq);
            if( x != 9999)
              {
                targetLoc = x * REC_LEN;
              }
